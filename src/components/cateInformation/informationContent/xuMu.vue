    ...
    <div class="all">
        ...
        <ul class="information-list">
            <li v-for="(n,i) in data" :key="i">
                <router-link :to="{path:'/cateInformationDetail',query:{value:n.id}}" class="router">
                    <p>{{n.title}}</p>
...
<script>
    export default {
        ...
        methods: {
            getInfo(){
                this.$axios.get(this.HOST+'news/get_news/?start='+this.start+'&end='+this.end+'&category_id=5')
                    .then((response)=>{
