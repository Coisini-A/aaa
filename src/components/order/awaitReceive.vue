    ...
    <div class="module">
        <div v-for="(data,i) in this.orderInfo.data" :key="i" class="bag">
            <div class="good">
                ...
                <div class="button"><button>查看物流</button><button @click="confirm(data.order_id)" class="obtn">确认收货</button></div>
            </div>
        </div>
...
<script>
    export default {
        ...
        methods:{
            afterRead() {
                fetch('http://39.98.112.201/order/get_orders/?order_status=2&start=0&end=20', {
                    method: 'GET',
                    headers: {
